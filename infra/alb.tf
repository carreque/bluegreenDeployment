# The environment's entire ingress path, and the file where enable_prod does the
# most work — this is the largest single difference between the two shapes.
#
# STAGING builds one target group and two listeners: :80 redirecting to :443,
# and :443 forwarding straight to that group. Nothing shifts, so nothing needs a
# listener rule.
#
# PRODUCTION builds a second target group, a third listener, and two listener
# rules. Two listeners carry traffic and one redirects. The production listener
# is :443 and the test listener is :8443 — the port network's prod ALB security group
...
resource "aws_lb" "this" {
  ...
  # checkov:skip=CKV_AWS_91:access logging is off in both environments, decided rather than inherited. Phase 5 §D5 deferred the production decision to Phase 6 on the grounds that access logs are genuine blue/green evidence; having looked at what the evidence actually needs to be, they are not. The three exit criteria are met by /version on :443 versus :8443 during a deployment, the ECS deployment-stage transitions, and the hook Lambdas own log output — all available within seconds. ALB access logs are delivered to S3 on a roughly five-minute lag, which is longer than the deployment they would document. Enabling them also requires either a bucket policy in foundation, a layer this is scoped not to touch, or a bucket that make teardown destroys along with every log in it. Phase 6 plan §D7.
...
resource "aws_lb_target_group" "blue" {
  ...
  # The suffix, not the whole name, is conditional. Staging's single group has
  # been "<prefix>-api" since Phase 5 and there is no second colour to
  # distinguish it from, so renaming it to "-api-blue" here would be a rename
  # this refactor has no reason to make. Production keeps "-api-blue".
  name        = var.enable_prod ? "${local.env_prefix}-api-blue" : "${local.env_prefix}-api"
...
resource "aws_lb_target_group" "green" {
  # The second colour exists only where something shifts traffic onto it.
...
resource "aws_lb_listener" "https" {
  ...
  # Staging forwards from the default action; production refuses from it.
  #
  # Production's rules are what carry traffic (advanced_configuration takes RULE
  # arns, per Phase 0 A7), each matching /*, so the default below is unreachable
  # while they exist — which is precisely why it should refuse rather than
  # forward. A forwarding default would keep serving from whichever target group
  # it named if a rule were ever removed, and the colour it named would be the
  # wrong one half the time.
  #
  # Staging has no rules and nothing to shift, so its default action IS the
  # route. Exactly one of these two renders.
