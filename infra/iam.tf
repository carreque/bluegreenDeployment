# Five roles in production, two in staging, of design §8.1's seven. Phase 0 found that §8.1's list of five was
...
#
# The first two are unconditional — every environment runs tasks that pull an
# image and write logs. The last three are gated on var.enable_prod: they exist
# to serve a traffic shift, and staging shifts nothing. A hook-invoke role with
# no hooks to invoke is not a smaller blast radius, it is a role nobody can
# explain.
#
# All are created here rather than in foundation because
...
resource "aws_iam_role_policy_attachment" "bluegreen" {
  ...
  role       = aws_iam_role.bluegreen[0].name
...
resource "aws_iam_role_policy" "hook_invoke" {
  ...
  role = aws_iam_role.hook_invoke[0].id
