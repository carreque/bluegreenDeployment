# ---------------------------------------------------------------------------
# What environment this apply is
# ---------------------------------------------------------------------------
#
# These two are the whole of the staging/production split. Everything else in
# this file is a size or a timeout.
#
# Both default to STAGING's value. An apply that forgets its -var-file builds a
# staging-shaped environment into whatever state key it was initialised with,
# which is recoverable; the opposite default would build production's ALB
# listeners, three Lambdas and four alarms by omission.

variable "environment" {
  description = <<-EOT
    Which environment this apply builds. A name segment in every resource name,
    a tag value, and the thing that picks staging's hostname over production's.

    Not derived from enable_prod, and not a boolean's second face: the naming
    convention (§3) treats this as a first-class segment, and a future third
    environment would set this without touching enable_prod's meaning.
  EOT
  type        = string
  default     = "staging"
  ...
  validation {
    ...
    error_message = "environment must be staging or prod; foundation exports a hostname for each and no others."
  }
}

variable "enable_prod" {
  description = <<-EOT
    Whether to build the blue/green machinery. False in staging, true in
    production.

    What it gates, and nothing else:

      alb.tf     the green target group, the :8443 test listener, and the two
                 aws_lb_listener_rules ECS rewrites mid-shift
      ecs.tf     BLUE_GREEN over ROLLING, the three lifecycle_hook blocks,
                 wait_for_steady_state, and the load_balancer block's
                 advanced_configuration
      iam.tf     the bluegreen and hook_invoke roles
      hooks.tf   all three Lambda functions
      alarms.tf  all four bake alarms

    Deliberately one flag rather than five. The five move together — a bake
    alarm with no bake to observe, or a hook role with no hooks to invoke, is
    not a configuration anybody wants — and five independent booleans would be
    thirty-two configurations of which two are meaningful.

    It is NOT the same question as `environment`. This one asks "build the
    blue/green shape"; that one asks "what is this called and where does it
    live". A staging environment that wanted blue/green would set this true and
    leave environment alone, and tests/mocks.tftest.hcl exercises exactly that
    pairing to prove the two are independent.
  EOT
  type        = bool
  default     = false
}
...
variable "desired_count" {
  description = <<-EOT
    How many tasks the service runs.

    One in staging, which exists to fail fast rather than to be available. Two
    in production, for two independent reasons: design §10 prices production at
    two tasks, and two tasks across two availability zones is the minimum that
    makes the UnHealthyHostCount bake alarm mean "one task is sick" rather than
    being a synonym for "the service is down". Phase 6 plan §D13.

    The default is staging's. Every env-varying default in this file defaults to
    the SMALLER, cheaper, non-production value and production overrides it in
    environments/prod.tfvars — so a forgotten -var-file under-provisions a
    staging-shaped environment rather than silently building a production-shaped
    one. enable_prod, below, follows the same rule for the same reason.
  EOT
  type        = number
  default     = 1
