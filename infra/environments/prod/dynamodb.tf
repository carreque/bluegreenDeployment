...
#
# On-demand billing rather than provisioned: an idle environment then costs
# nothing at all, which is what makes leaving it up between sessions a decision
# about the ALB and NAT rather than about the tables.
#
# These tables are prod's own. They are deliberately not shared with staging —
# separate data per environment is the point of having two environments, and
# roadmap §1's layer diagram puts DynamoDB under envs/prod/ even though the
# Phase 6 task list omits it. Plan §D13.

resource "aws_dynamodb_table" "accounts" {
  # checkov:skip=CKV_AWS_28:point-in-time recovery buys nothing on tables that make teardown destroys and rebuild recreates empty. There is no point in time worth recovering to. Phase 5 plan §D6.
...
resource "aws_dynamodb_table" "transactions" {
  # checkov:skip=CKV_AWS_28:as above — Phase 5 plan §D6.
