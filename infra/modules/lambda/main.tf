...
resource "aws_lambda_function" "this" {
  # checkov:skip=CKV_AWS_50:X-Ray traces a request as it crosses services. This function calls exactly one thing, over one http.client connection, and already logs the stage, the URL, the connect and per-request timings, the probe outcome and the digest it saw. A trace would add a sampling charge and a segment for a call the log line already describes in full, and the thing an operator actually needs after a rejected deployment is the exception message naming the path and status — which is in CloudWatch, not in a trace.
