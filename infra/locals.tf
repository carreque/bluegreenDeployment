...
# is staging or prod. Derived strings are rebuilt locally; only real identifiers
...
locals {
  # var.environment, not a literal, and not derived from var.enable_prod. This
  # is the single line that used to be the difference between two whole
  # directories of otherwise-identical Terraform.
  environment = var.environment
  env_prefix  = "${var.project_name}-${var.region}-${local.environment}"

  # foundation exports a hostname per environment under two different output
  # names rather than one map, so the choice is made here rather than at the
  # three call sites (dns.tf, and both outputs). Keyed on var.environment
  # rather than var.enable_prod: this is a question about which hostname, not
  # about which deployment strategy.
  api_domain = var.environment == "prod" ? local.foundation.api_domain : local.foundation.staging_api_domain
