...
run "the_target_group_health_checks_liveness_only" {
  ...
  assert {
    condition     = aws_lb_target_group.blue.name == "bgd-us-east-1-staging-api" && length(aws_lb_target_group.blue.name) <= 32
  ...
  assert {
    condition     = aws_lb_target_group.blue.target_type == "ip"
  ...
  assert {
    condition     = one(aws_lb_target_group.blue.health_check).path == "/health"
    error_message = "the health check must poll /health; /ready would deregister every task on a DynamoDB hiccup"
  }

  assert {
    condition     = aws_lb_target_group.blue.port == 8080
  ...
  assert {
    condition     = aws_lb_target_group.blue.deregistration_delay == "30"
