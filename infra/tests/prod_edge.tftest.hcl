...
run "the_two_target_groups_are_symmetric_and_health_check_liveness_only" {
  ...
  assert {
    condition = (
      ...
      aws_lb_target_group.green[0].name == "bgd-us-east-1-prod-api-green"
    )
  ...
  assert {
    condition = alltrue([
      ...
      length(aws_lb_target_group.green[0].name) <= 32,
    ])
  ...
  assert {
    condition = (
      aws_lb_target_group.blue.port == aws_lb_target_group.green[0].port &&
      aws_lb_target_group.blue.protocol == aws_lb_target_group.green[0].protocol &&
      aws_lb_target_group.blue.target_type == aws_lb_target_group.green[0].target_type &&
      aws_lb_target_group.blue.vpc_id == aws_lb_target_group.green[0].vpc_id &&
      aws_lb_target_group.blue.deregistration_delay == aws_lb_target_group.green[0].deregistration_delay
    )
  ...
  assert {
    condition = (
      one(aws_lb_target_group.blue.health_check).path == one(aws_lb_target_group.green[0].health_check).path &&
      one(aws_lb_target_group.blue.health_check).interval == one(aws_lb_target_group.green[0].health_check).interval &&
      one(aws_lb_target_group.blue.health_check).healthy_threshold == one(aws_lb_target_group.green[0].health_check).healthy_threshold &&
      one(aws_lb_target_group.blue.health_check).unhealthy_threshold == one(aws_lb_target_group.green[0].health_check).unhealthy_threshold
    )
  ...
  assert {
    condition = (
      ...
      one(aws_lb_target_group.green[0].health_check).path == "/health"
    )
...
run "there_are_three_listeners_and_only_two_serve" {
  ...
  assert {
    condition = (
      ...
      aws_lb_listener.test[0].port == 8443
    )
  ...
  assert {
    condition = (
      ...
      aws_lb_listener.test[0].certificate_arn == "arn:aws:acm:us-east-1:590184028094:certificate/mock"
    )
  ...
  assert {
    condition = (
      ...
      aws_lb_listener.test[0].ssl_policy == "ELBSecurityPolicy-TLS13-1-2-2021-06"
    )
  ...
  assert {
    condition = (
      ...
      one(aws_lb_listener.test[0].default_action).type == "fixed-response"
    )
  ...
  assert {
    condition = (
      ...
      one(one(aws_lb_listener.test[0].default_action).fixed_response).status_code == "503"
    )
...
run "each_serving_listener_carries_a_rule_and_they_start_on_opposite_colours" {
  ...
  assert {
    condition     = one(aws_lb_listener_rule.production[0].action).target_group_arn == aws_lb_target_group.blue.arn
    error_message = "the :443 rule forwards to blue at creation; ECS swaps it from there"
  }

  assert {
    condition     = one(aws_lb_listener_rule.test[0].action).target_group_arn == aws_lb_target_group.green[0].arn
    error_message = "the :8443 rule forwards to green at creation, so the test listener starts on the idle colour"
  }

  assert {
    condition = (
      aws_lb_listener_rule.production[0].listener_arn == aws_lb_listener.https.arn &&
      aws_lb_listener_rule.test[0].listener_arn == aws_lb_listener.test[0].arn
    )
  ...
  assert {
    condition = (
      one(one(aws_lb_listener_rule.production[0].condition).path_pattern).values == toset(["/*"]) &&
      one(one(aws_lb_listener_rule.test[0].condition).path_pattern).values == toset(["/*"])
    )
  ...
  assert {
    condition = (
      aws_lb_listener_rule.production[0].priority != null &&
      aws_lb_listener_rule.test[0].priority != null
    )
