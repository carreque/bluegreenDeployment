...
override_resource {
  target = aws_iam_role.bluegreen[0]
  values = {
...
override_resource {
  target = aws_iam_role.hook_invoke[0]
  values = {
...
run "there_are_three_hooks_named_by_convention" {
  ...
  assert {
    condition = toset([
      module.pre_scale_hook[0].function_name,
      module.post_test_hook[0].function_name,
      module.post_prod_hook[0].function_name,
      ]) == toset([
  ...
  assert {
    condition = (
      module.pre_scale_hook[0].function_name == local.hook_function_names.pre_scale &&
      module.post_test_hook[0].function_name == local.hook_function_names.post_test &&
      module.post_prod_hook[0].function_name == local.hook_function_names.post_prod
    )
...
run "the_hooks_run_the_runtime_and_architecture_the_project_pins" {
  ...
  assert {
    condition = alltrue([
      module.pre_scale_hook[0].function_arn != "",
      module.post_test_hook[0].function_arn != "",
      module.post_prod_hook[0].function_arn != "",
    ])
  ...
  assert {
    condition = toset([
      module.pre_scale_hook[0].log_group_name,
      module.post_test_hook[0].log_group_name,
      module.post_prod_hook[0].log_group_name,
      ]) == toset([
  ...
  assert {
    condition = alltrue([
      for role in [
        module.pre_scale_hook[0].role_name,
        module.post_test_hook[0].role_name,
        module.post_prod_hook[0].role_name,
      ] : role != aws_iam_role.hook_invoke[0].name
    ])
...
run "the_hooks_pin_the_runtime_and_the_architecture" {
  ...
  assert {
    condition = alltrue([
      module.pre_scale_hook[0].runtime == "python3.14",
      module.post_test_hook[0].runtime == "python3.14",
      module.post_prod_hook[0].runtime == "python3.14",
    ])
  ...
  assert {
    condition = alltrue([
      module.pre_scale_hook[0].architectures == tolist(["arm64"]),
      module.post_test_hook[0].architectures == tolist(["arm64"]),
      module.post_prod_hook[0].architectures == tolist(["arm64"]),
    ])
  ...
  assert {
    condition = alltrue([
      module.pre_scale_hook[0].timeout_seconds == var.hook_timeout_seconds,
      module.post_test_hook[0].timeout_seconds == var.hook_timeout_seconds,
      module.post_prod_hook[0].timeout_seconds == var.hook_timeout_seconds,
    ])
...
run "the_post_test_hook_probes_the_test_listener_and_the_others_do_not" {
  ...
  assert {
    condition     = module.post_test_hook[0].environment_variables.BGD_PROBE_URL == "https://api.carloscloudengineer.com:8443"
    error_message = "the post-test hook must probe the :8443 test listener; :443 would validate the colour already serving"
  }

  assert {
    condition = (
      module.pre_scale_hook[0].environment_variables.BGD_PROBE_URL == "https://api.carloscloudengineer.com" &&
      module.post_prod_hook[0].environment_variables.BGD_PROBE_URL == "https://api.carloscloudengineer.com"
    )
  ...
  assert {
    condition = alltrue([
      for env in [
        module.pre_scale_hook[0].environment_variables,
        module.post_test_hook[0].environment_variables,
        module.post_prod_hook[0].environment_variables,
...
run "each_hook_knows_which_stage_it_is" {
  ...
  assert {
    condition = (
      module.pre_scale_hook[0].environment_variables.BGD_STAGE == "PRE_SCALE_UP" &&
      module.post_test_hook[0].environment_variables.BGD_STAGE == "POST_TEST_TRAFFIC_SHIFT" &&
      module.post_prod_hook[0].environment_variables.BGD_STAGE == "POST_PRODUCTION_TRAFFIC_SHIFT"
    )
...
run "only_the_pre_scale_hook_tolerates_an_unserved_endpoint" {
  ...
  assert {
    condition     = module.pre_scale_hook[0].environment_variables.BGD_ALLOW_UNSERVED == "true"
...
run "there_are_exactly_four_bake_alarms_named_by_convention" {
  ...
  assert {
    condition = toset(local.bake_alarm_names) == toset([
      aws_cloudwatch_metric_alarm.five_xx[0].alarm_name,
      aws_cloudwatch_metric_alarm.p95_latency[0].alarm_name,
...
run "the_user_facing_alarms_measure_the_whole_load_balancer" {
  ...
  assert {
    condition = (
      toset(keys(aws_cloudwatch_metric_alarm.five_xx[0].dimensions)) == toset(["LoadBalancer"]) &&
      toset(keys(aws_cloudwatch_metric_alarm.p95_latency[0].dimensions)) == toset(["LoadBalancer"])
    )
  ...
  assert {
    condition = (
      aws_cloudwatch_metric_alarm.five_xx[0].metric_name == "HTTPCode_Target_5XX_Count" &&
      aws_cloudwatch_metric_alarm.p95_latency[0].metric_name == "TargetResponseTime"
    )
  ...
  assert {
    condition = (
      aws_cloudwatch_metric_alarm.p95_latency[0].extended_statistic == "p95" &&
      (aws_cloudwatch_metric_alarm.p95_latency[0].statistic == null || aws_cloudwatch_metric_alarm.p95_latency[0].statistic == "")
    )
    error_message = "the latency alarm must use extended_statistic p95; an average hides the tail"
  }

  assert {
    condition     = aws_cloudwatch_metric_alarm.p95_latency[0].threshold == var.alarm_p95_seconds
...
run "every_alarm_can_evaluate_inside_a_five_minute_bake" {
  ...
  assert {
    condition = alltrue([
      for alarm in [
        aws_cloudwatch_metric_alarm.five_xx[0],
        aws_cloudwatch_metric_alarm.p95_latency[0],
...
run "there_are_three_lifecycle_hooks_one_per_stage" {
  ...
  assert {
    condition = alltrue([
      ...
      hook.hook_target_arn == lookup({
        PRE_SCALE_UP                  = module.pre_scale_hook[0].function_arn
        POST_TEST_TRAFFIC_SHIFT       = module.post_test_hook[0].function_arn
        POST_PRODUCTION_TRAFFIC_SHIFT = module.post_prod_hook[0].function_arn
  ...
  assert {
    condition = alltrue([
      for hook in one(aws_ecs_service.api.deployment_configuration).lifecycle_hook :
      hook.role_arn == aws_iam_role.hook_invoke[0].arn && hook.role_arn != aws_iam_role.bluegreen[0].arn
    ])
...
run "the_load_balancer_block_names_both_colours_and_both_rules" {
  ...
  assert {
    condition = (
      ...
      one(one(aws_ecs_service.api.load_balancer).advanced_configuration).alternate_target_group_arn == aws_lb_target_group.green[0].arn
    )
  ...
  assert {
    condition = (
      one(one(aws_ecs_service.api.load_balancer).advanced_configuration).production_listener_rule == aws_lb_listener_rule.production[0].arn &&
      one(one(aws_ecs_service.api.load_balancer).advanced_configuration).test_listener_rule == aws_lb_listener_rule.test[0].arn
    )
  ...
  assert {
    condition = (
      one(one(aws_ecs_service.api.load_balancer).advanced_configuration).role_arn == aws_iam_role.bluegreen[0].arn &&
      one(one(aws_ecs_service.api.load_balancer).advanced_configuration).role_arn != aws_iam_role.hook_invoke[0].arn
    )
