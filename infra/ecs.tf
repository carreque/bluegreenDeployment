...
resource "aws_ecs_service" "api" {
  ...
  #
  # Unconditional, and staging's reason is its own rather than an inheritance:
  # without this a rolling apply returns while the replacement is still in
  # flight, and Phase 8's Smoke action then reads the PREVIOUS task's /version
  # and fails a deployment that was in fact healthy. Both environments want the
  # apply to mean "the deployment finished".
  wait_for_steady_state = true

  # The whole of blue/green, in one block, and the single most important thing
  # enable_prod switches. ROLLING is also the API default; it is stated rather
  # than omitted because this one word is the difference between the two
  # environments, and a difference that matters should be visible rather than
  # implied by an absence. Phase 5 plan §F7.
  deployment_configuration {
    strategy = var.enable_prod ? "BLUE_GREEN" : "ROLLING"

    # tostring, because bake_time_in_minutes is typed *string* in the provider
    # schema and the number form fails terraform validate. Plan §F1. null under
    # ROLLING, where there is no bake to time.
    bake_time_in_minutes = var.enable_prod ? tostring(var.bake_time_minutes) : null
    ...
    # A SET of three, one per stage, and empty under ROLLING. Each names the
    # function built for that stage — crossed, the dark canary would probe the
    # production listener, validate the colour already serving, and approve
    # every bad build. Two tests in different runs assert the pairing from
    # opposite ends.
    #
    # Driven from local.lifecycle_hooks rather than three hand-written blocks,
    # so the stage-to-function pairing is stated once, in hooks.tf, beside the
    # functions themselves. Written out three times here, one of them would
    # eventually be edited alone.
    dynamic "lifecycle_hook" {
      for_each = local.lifecycle_hooks

      content {
        hook_target_arn  = lifecycle_hook.value.function_arn
        role_arn         = aws_iam_role.hook_invoke[0].arn
        lifecycle_stages = [lifecycle_hook.value.stage]
      }
    }
  }

  # Staging only, and its absence in production is a decision rather than a gap.
  #
  # In staging this is the whole rollback mechanism: without it a task that never
  ...
  # ongoing one. The trade accepted is that the broken task set is gone before it
  # can be inspected — the log group keeps what the container printed, which is
  # the part worth reading. Phase 5 plan §D8.
  #
  # In production the bake period with alarms IS the rollback mechanism, and
  # whether the two interact — and in what order they would each try to revert —
  # is not documented in the schema and not something to discover during a
  # production shift. One rollback mechanism, chosen on purpose. A test asserts
  # the omission under enable_prod so it keeps reading as a decision.
  dynamic "deployment_circuit_breaker" {
    for_each = var.enable_prod ? [] : [1]
  ...
  load_balancer {
    ...
    # Production only. In staging the :443 listener's own default action carries
    # traffic to this one target group, there is no second colour and no rule to
    # name, so the whole block is absent rather than empty.
    dynamic "advanced_configuration" {
      ...
      content {
        ...
        # (Phase 0 A7), and alb.tf's two aws_lb_listener_rule resources exist
        # for these two lines alone.
        production_listener_rule = aws_lb_listener_rule.production[0].arn
        test_listener_rule       = aws_lb_listener_rule.test[0].arn

        role_arn = aws_iam_role.bluegreen[0].arn
      }
    }
  }
  ...
  # Production only, because local.bake_alarm_names is empty in staging and an
  # alarms block listing nothing would declare a gate that gates nothing —
  # enable = true over an empty set, which the apply accepts in silence.
  dynamic "alarms" {
    for_each = var.enable_prod ? [1] : []
