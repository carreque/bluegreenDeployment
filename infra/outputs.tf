# The interface Phases 8, 9 and 11, the runbooks and scripts/smoke.sh all read.
...
#
# Every output is declared in BOTH environments. The blue/green ones resolve to
# null or [] when enable_prod is false rather than disappearing, and that is the
# whole reason they are written as splats and conditionals below.
#
# The alternative — gating the output blocks themselves — is not available:
...
# whichever environment they were pointed at, so a missing name is a red stage;
# a null is a value a caller can test.
...
output "test_url" {
  description = <<-EOT
    ...
    null in staging, which has no :8443 listener to point at.
  EOT
  value       = var.enable_prod ? "https://${local.api_domain}:8443" : null
}
...
output "green_target_group_arn" {
  description = "The target group the :8443 rule forwards to at creation. Published alongside blue because a traffic shift is only observable if you can name both colours. null in staging, which has one colour. one() rather than [0] so the empty case is null instead of an index error."
...
output "hook_function_names" {
  description = <<-EOT
    ...
    The empty list in staging, which runs no hooks.
  EOT
  value       = var.enable_prod ? values(local.hook_function_names) : []
}
...
output "bake_alarm_names" {
  description = <<-EOT
    ...
    The empty list in staging, which has no bake to gate.
  EOT
