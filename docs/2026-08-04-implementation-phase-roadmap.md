...
| **Build-time pulls from shared CI egress are rate-limited by third parties** | Found twice on 2026-08-31, minutes apart: `api.github.com` (60/hour/IP) installing the tflint ruleset, then Docker Hub (unauthenticated pull limit) fetching checkov. Both are per-source-IP, both are invisible on a laptop with its own budget, and both fail a pipeline for reasons unrelated to the code. Mitigated by preferring registries with no anonymous limit — `ghcr.io`, `quay.io` — and by fetching release assets from CDNs rather than through APIs. **Two Docker Hub pulls remain**, `python:3.14.6-slim` and `amazon/dynamodb-local`, both in the application pipeline's Build stage; moving them needs digests re-recorded and `make image-verify` re-run, so it is deliberate work rather than an incident fix. |
